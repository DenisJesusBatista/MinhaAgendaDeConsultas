...
namespace MinhaAgendaDeConsultas.Application.UseCases.AgendamentoConsultas.Validadores
{
    public class AgendamentoConsultasValidacao : AbstractValidator<RequisicaoAgendamentoConsultasJson>
    {
        public AgendamentoConsultasValidacao()
        {
            ...
            RuleFor(agendamento => agendamento.MedicoEmail).NotEmpty().WithMessage(ResourceMessagesExceptions.MEDICO_NAO_INFORMADO);
            RuleFor(agendamento => agendamento.PacienteEmail).NotEmpty().WithMessage(ResourceMessagesExceptions.PACIENTE_DEVE_SER_INFORMADO);
